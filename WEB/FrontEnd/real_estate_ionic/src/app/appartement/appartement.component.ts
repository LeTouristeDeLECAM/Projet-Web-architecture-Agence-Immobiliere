...
export class AppartementComponent {  //implements OnInit {
  ...
  editAppart(appart_Id:number ){
    // this.propertyManagementService.editAppartement( appartement).subscribe();
    this.router.navigate(['appartement/edit', appart_Id]);
  ...
  showTicketList(appart_Id: number){
    ...
    this.router.navigateByUrl('/ticket/' + appart_Id);
    console.log(" /ticket/", appart_Id);
  }
  ...
  getTicketById(){
    console.log("appart_Id:", this.searchForm.value.ticket_Id);
    this.router.navigate(['/ticket', this.searchForm.value.ticket_Id]);
  }
